...
def get_output_dict(
    dataset, subject, events_to_load, srate_in, srate_out,target_session_idx, fmin, fmax, verbose=0
):
    ...
    x, y, labels, metadata, channels, adjacency_mtx, srate = load_data(
        paradigm, dataset, [subject], target_session_idx
    )
...
def load_data(paradigm, dataset, idx, session_id=None):
    ...
    # Filter sessions if session_id is provided
    if session_id is not None:
        x = x[session_id]
        labels = labels[session_id]
        metadata = metadata[metadata['session'] == session_id]
