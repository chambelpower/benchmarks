...
class EMDGenerator(nn.Module):
    ...
    def compute_envelopes(self, signal, maxima, minima):
        ...
        if len(maxima) < 4 or len(minima) < 4:
            ...
            kind = 'cubic'
        upper_env = si.interp1d(maxima, signal[maxima], kind=kind, fill_value='extrapolate')(t) if len(maxima) > 1 else np.zeros_like(signal)
        lower_env = si.interp1d(minima, signal[minima], kind=kind, fill_value='extrapolate')(t) if len(minima) > 1 else np.zeros_like(signal)
    ...
    def decompose_eeg_frames(self, eeg_frames):
        device = eeg_frames.device  # Get the device of the input tensor
        decomposed_frames = []
        for frame in eeg_frames.cpu().numpy():  # Temporarily move data to CPU for processing
            decomposed_channels = []
            for channel in frame:
                imfs = self.emd(channel)
        ...
        return torch.tensor(decomposed_frames_np, dtype=eeg_frames.dtype).to(device)  # Move back to the original device

    def forward(self, waveforms):
        # Get the batch size of the input
        original_batch_size = waveforms.shape[0]
        self.num_artificial_frames = original_batch_size
        device = waveforms.device  # Get the device of the input tensor
        ...
        for _ in range(self.num_artificial_frames):
            artificial_frame = torch.zeros((num_channels, num_samples), dtype=waveforms.dtype, device=device)
